...
def extract_stories_from_NPR_text(text):
    ...
    titleag =div_tag.find('h3',title')
    teasertagdiv_tag.find('p',teaser')

     if teasertag !=None:
         stories.append((titletag.text,teasertag.text))
     else:
         storiesstories.append((titletag.text,"" )) 
...
def read_nth_story(stories, n, filename):
    ...
    Output: None
    '''
     gtts.gTTS(text=stories[n][0]+""+stories[n][1] ,lang="en").save(fiename)                    
    #raise RuntimeError('You need to write this part!')